...
export class ImportDataService {
   ...
   validDataTypes = ['string', 'integer', 'float', 'date'];
    ...
    */
   validateTableName(tableName: string): { result: boolean; message: string } {
      console.log('validateTableName');
     ...
     if (!tableNameRegex.test(tableName)) {
       return {
         result: false,
         message: 'Invalid table name. It must start with a letter or underscore and contain only alphanumeric characters or underscores. It cannot exceed 64 characters.',
       };
     }
    ...
    * @returns Object with result (boolean) and message (string).
    */
   validateColumnNames(columnNames: string[]): { result: boolean; message: string } {
    console.log('validateColumnNames');
    ...
    * @returns Object with result (boolean) and message (string).
    */
   validateDataTypes(data: any[], selectedDataTypes: string[], sampleSize: number = 500): { result: boolean; message: string } {
